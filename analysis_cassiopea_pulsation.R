...
missing_summary <- data.frame(
  variable = c("ph", "oxygen_mg_l", "z_ph", "z_ox"),
  n_missing = c(
    sum(is.na(df$ph)),
    sum(is.na(df$oxygen_mg_l)),
    sum(is.na(df$z_ph)),
    sum(is.na(df$z_ox))
  )
)
cat("\nMissing values in pH/oxygen variables and their standardized versions:\n")
...
# Nominal phases are kept only for sensitivity analyses.
tp_codes <- c(Dawn = 6, Noon = 12, Dusk = 18, Midnight = 0)

df <- df |>
  mutate(
    phase24_nominal = unname(tp_codes[as.character(time_point)]),
    phase24_real = if_else(is.na(time_med_h), phase24_nominal, time_med_h) %% 24,
    day_num = as.integer(gsub("^D", "", as.character(day))) - 1L,

    # Midnight observations around 00:00 belong to the end of the experimental day.
    phase_real_t72 = if_else(time_point == "Midnight" & phase24_real < 6,
                             phase24_real + 24,
                             phase24_real),
    phase_nominal_t72 = if_else(time_point == "Midnight", 24, phase24_nominal),
    t72_real = day_num * 24 + phase_real_t72,
    t72_nominal = day_num * 24 + phase_nominal_t72,

    # Plot midnight on the right-hand edge of 24 h figures.
    phase24_real_plot = if_else(time_point == "Midnight" & phase24_real < 6,
                                24,
                                phase24_real)
  )
...
phase_breaks <- df |>
  group_by(time_point) |>
  summarise(phase = median(phase24_real_plot, na.rm = TRUE), .groups = "drop") |>
  arrange(time_point)

cat("\nMain analysis uses real observation times from time_med.\n")
cat("Sensitivity analysis uses nominal cycle phases (Dawn=6, Noon=12, Dusk=18, Midnight=24 for 72 h time).\n")
...
df_plot <- df |>
  arrange(individual_id, t72_real) |>
  mutate(tp72 = paste(day, time_point, sep = "-"))

df_sum72 <- df_plot |>
  group_by(day, time_point, tp72) |>
  summarise(
    t72_real_mid = median(t72_real, na.rm = TRUE),
  ...
  arrange(t72_real_mid)

p_fig72 <- ggplot() +
  geom_line(data = df_plot,
            aes(x = t72_real, y = pulsation_frequency, group = individual_id),
            color = "grey75", alpha = 0.5, linewidth = 0.5) +
  geom_point(data = df_plot,
             aes(x = t72_real, y = pulsation_frequency),
             color = "grey60", size = 0.9, alpha = 0.35) +
  geom_line(data = df_sum72,
            aes(x = t72_real_mid, y = mean_freq, group = 1),
            color = "#8F9779", linewidth = 1.6) +
  geom_point(data = df_sum72,
             aes(x = t72_real_mid, y = mean_freq),
             color = "#556B2F", size = 2.4) +
  geom_errorbar(data = df_sum72,
                aes(x = t72_real_mid, ymin = mean_freq - se_freq, ymax = mean_freq + se_freq),
                width = 0.12, color = "#8F9779", linewidth = 0.6) +
  scale_x_continuous(breaks = df_sum72$t72_real_mid, labels = df_sum72$tp72) +
  labs(
    ...
    x = "Experimental time (h; real observation times)",
    y = "Pulsation frequency (pulse/min)"
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
print(p_fig72)
ggsave(file.path(fig_dir, "Figure2_pulsation_72h.png"), p_fig72,
...
print(p_fig_lmm)
ggsave(file.path(fig_dir, "Figure3_lmm_cycle_phase.png"), p_fig_lmm,
...
m_gam24 <- mgcv::gam(
  log(pulsation_frequency) ~ s(phase24_real, bs = "cc", # Cyclic cubic regression spline
  ...
  data  = df,
  method = "REML",
  knots = list(phase24_real = c(0, 24)) # Ensuring continuity at the 0–24 h boundary
)
...
pred24_gam <- data.frame(
  phase24_real = seq(0, 24, length.out = 721),
...
newdata_gam <- data.frame(
  phase24_real = seq(0, 24, length.out = 900)
) %>%
  mutate(
...
peak_time_gam <- newdata_gam$phase24_real[which.max(newdata_gam$fit)]
...
get_peak_gam <- function(sim) newdata_gam$phase24_real[which.max(sim)]
...
pred24_gam <- pred24_gam %>%
  mutate(phase24_real_plot = if_else(phase24_real == 0, 24, phase24_real))

peak_time_plot <- peak_time_gam
peak_CI_plot   <- peak_CI_gam

## ---- Nocturnal minimum 95% CI by coefficient simulation  ----
low_time_gam <- newdata_gam$phase24_real[which.min(newdata_gam$fit)]

get_low_gam <- function(sim) newdata_gam$phase24_real[which.min(sim)]
...
p_fig_gam24 <- ggplot(pred24_gam, aes(phase24_real_plot, fit)) +
  ...
  geom_point(data = df, aes(y = pulsation_frequency, x = phase24_real_plot), alpha = 0.45) +
  ...
  # Axis: display cycle phases at their median real observation times.
  scale_x_continuous(
    breaks = x_breaks_gam,
    labels = x_labels_gam,
    limits = c(0, 24)
...
print(p_fig_gam24)
ggsave(file.path(fig_dir, "Figure4_cyclic_gamm_24h.png"), p_fig_gam24,
...
# detrend first
y_dt <- resid(lm(mean_freq ~ t72_real, data = df_mean))

# Lomb–Scargle periodogram (period in hours; main analysis uses real observation times)
ls <- lomb::lsp(
  x = y_dt,
  times = df_mean$t72_real,
  type = "period",
  from = 12, to = 72,
  # Dense frequency sampling for a publication-style periodogram.
  # This is not a statistical smoother; it only evaluates the Lomb-Scargle
  # spectrum on a finer period grid.
  ofac = 80,
  plot = FALSE
)
...
p_fig_lomb <- ggplot(spec_df, aes(x = period, y = power_rel)) +
...
print(p_fig_lomb)
ggsave(file.path(fig_dir, "Figure5_lomb_scargle.png"), p_fig_lomb,
...
rpt_res <- rptR::rptGaussian(
  formula = log(pulsation_frequency) ~ time_point + z_umbrella + (1 | individual_id) + (1 | day),
...
# Additional analyses not shown in main text 
########################################################### #
...
path <- "cassiopea_pulsation_dataset.xlsx"
df_sens <- read_excel(path) |> clean_names()

# (Re)build nominal time variables for sensitivity analyses.
tp_codes <- c(Dawn = 6, Noon = 12, Dusk = 18, Midnight = 0)

df_sens$individual_id <- factor(df_sens$individual_id)
df_sens$day <- as.character(df_sens$day)
df_sens$day <- if_else(grepl("^D", df_sens$day), df_sens$day, paste0("D", df_sens$day))
df_sens$day <- factor(df_sens$day)
df_sens$time_point <- factor(df_sens$time_point, levels = c("Dawn", "Noon", "Dusk", "Midnight"))
df_sens$pulsation_frequency <- as.numeric(df_sens$pulsation_frequency)
df_sens$umbrella_diameter_cm <- as.numeric(df_sens$umbrella_diameter_cm)
df_sens$z_umbrella <- as.numeric(scale(df_sens$umbrella_diameter_cm))
df_sens$phase24_nominal <- unname(tp_codes[as.character(df_sens$time_point)])
df_sens$day_num <- as.integer(gsub("^D", "", as.character(df_sens$day))) - 1L
df_sens$phase_nominal_t72 <- if_else(df_sens$time_point == "Midnight", 24, df_sens$phase24_nominal)
df_sens$t72_nominal <- df_sens$day_num * 24 + df_sens$phase_nominal_t72

########################### #
## A) GAM 72 h nominal-phase sensitivity ####
########################### #
form_gam72_nominal <- log(pulsation_frequency) ~ s(t72_nominal, k = 12, bs = "cr") + z_umbrella + s(individual_id, bs = "re") + s(day, bs = "re")
m_gam72_nominal <- mgcv::gam(form_gam72_nominal, data = df_sens, method = "REML")
summary(m_gam72_nominal)
mgcv::gam.check(m_gam72_nominal)

########################### #
## B) Direct nominal-phase sensitivity for the 24 h GAMM ####
########################### #
# This mirrors the main 24 h GAMM, but uses fixed nominal phases
# (Dawn = 6, Noon = 12, Dusk = 18, Midnight = 0).
# k = 4 is used because the nominal coding has only four unique time values.
m_gam24_nominal <- mgcv::gam(
  log(pulsation_frequency) ~ s(phase24_nominal, bs = "cc", k = 4) +
    z_umbrella +
    s(individual_id, bs = "re") +
    s(day, bs = "re"),
  data = df_sens,
  method = "REML",
  knots = list(phase24_nominal = c(0, 24))
)
summary(m_gam24_nominal)
mgcv::gam.check(m_gam24_nominal)

pred24_nominal <- data.frame(
  phase24_nominal = seq(0, 24, length.out = 721),
  z_umbrella = mean(df_sens$z_umbrella, na.rm = TRUE),
  individual_id = levels(df_sens$individual_id)[1],
  day = levels(df_sens$day)[1]
)
pred24_nominal$fit <- exp(predict(
  m_gam24_nominal,
  newdata = pred24_nominal,
  exclude = c("s(individual_id)", "s(day)")
))
cat(sprintf(
  "\nNominal-phase GAMM sensitivity: Peak=%.1f h, Trough=%.1f h\n",
  pred24_nominal$phase24_nominal[which.max(pred24_nominal$fit)],
  pred24_nominal$phase24_nominal[which.min(pred24_nominal$fit)]
))

########################### #
## C) Direct nominal-phase sensitivity for Lomb-Scargle ####
########################### #
df_mean_nominal <- df_sens |>
  group_by(t72_nominal) |>
  summarise(mean_freq = geommean(pulsation_frequency), .groups = "drop") |>
  arrange(t72_nominal)

y_dt_nominal <- resid(lm(mean_freq ~ t72_nominal, data = df_mean_nominal))
ls_nominal <- lomb::lsp(
  x = y_dt_nominal,
  times = df_mean_nominal$t72_nominal,
  type = "period",
  from = 12,
  to = 72,
  ofac = 80,
...
spec_nominal_df <- data.frame(period = ls_nominal$scanned, power = ls_nominal$power) |>
  mutate(power_rel = power / max(power, na.rm = TRUE))
peak_nominal <- spec_nominal_df |> slice_max(power, n = 1)
cat("\nNominal-phase Lomb-Scargle sensitivity peak:\n")
print(peak_nominal)

########################### #
## D) Cosinor models using nominal phases (12 h, 24 h, 24 h + 12 h harmonic) ####
########################### #
df_sens$w <- 2 * pi * (df_sens$phase24_nominal %% 24) / 24
df_sens$sin_w <- sin(df_sens$w)
df_sens$cos_w <- cos(df_sens$w)

form_cosinor_12_nominal <- log(pulsation_frequency) ~ sin(2 * w) + cos(2 * w) + z_umbrella + s(individual_id, bs = "re") + s(day, bs = "re")
form_cosinor_24_nominal <- log(pulsation_frequency) ~ sin_w + cos_w + z_umbrella + s(individual_id, bs = "re") + s(day, bs = "re")
form_cosinor_24_12_nominal <- log(pulsation_frequency) ~ sin_w + cos_w + sin(2 * w) + cos(2 * w) + z_umbrella + s(individual_id, bs = "re") + s(day, bs = "re")

m_cosinor_12_nominal <- mgcv::gam(form_cosinor_12_nominal, data = df_sens, method = "REML")
m_cosinor_24_nominal <- mgcv::gam(form_cosinor_24_nominal, data = df_sens, method = "REML")
m_cosinor_24_12_nominal <- mgcv::gam(form_cosinor_24_12_nominal, data = df_sens, method = "REML")

AIC(m_cosinor_12_nominal, m_cosinor_24_nominal, m_cosinor_24_12_nominal)
anova(m_cosinor_24_nominal, m_cosinor_24_12_nominal, test = "Chisq")
anova(m_cosinor_12_nominal, m_cosinor_24_12_nominal, test = "Chisq")

########################### #
## E) Alternative spectral approach: Fourier on regularized mean series ####
########################### #
df_mean <- df_sens |>
  group_by(t72_nominal) |>
  summarise(mean_freq = geommean(pulsation_frequency), .groups = "drop") |>
